...
MAX_PAGES = 60  # safety cap (~1,500 jobs at 25/page)
...
class Scraper(BaseScraper):
    async def scrape(self) -> list[Job]:
        async with httpx.AsyncClient(
            ...
            # Fetch search form page to get the esearch SID
            r_home = await client.get(JOBS_URL)
            r_home.raise_for_status()
            soup_home = BeautifulSoup(r_home.text, "lxml")
            esearch_form = soup_home.find("form", {"action": lambda x: x and "esearch" in x})
            if not esearch_form:
                raise RuntimeError("Could not find esearch form on home page")
            esearch_action = esearch_form["action"]
            search_sid = esearch_form.find("input", {"name": "SID"})["value"]

            # POST blank search to get all jobs
            form_data = {
            ...
            while page_num < MAX_PAGES:
                ...
                if "Quick Check Needed" in r_results.text:
                    self.log.warning("ALTCHA re-triggered, stopping pagination")
                    break
                ...
                try:
                    ...
                    self.log.error(f"Page {page_num + 1} fetch failed: {e}")
                    break

            self.log.info(f"Total: {len(jobs)} jobs across {page_num} pages")
            return jobs
